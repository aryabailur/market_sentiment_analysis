...
**Fear/Greed Index** — one record per day with a score from 0–100 and a category label (Extreme Fear → Extreme Greed).
...
- Merged both datasets on date — ended up with 211,218 rows (lost 6 rows because Oct 26 2024 had no Fear/Greed record, which is fine)
- Filtered to closed trades only (Closed PnL ≠ 0) → 104,402 rows
...
One thing I caught during this — I initially included `pnl_efficiency` as a feature and got ROC-AUC of 1.0. That's data leakage since efficiency is calculated directly from Closed PnL (the target). Dropped it and retrained.
...
The raw sentiment score (0–100) was more predictive than the category label, which makes sense — whether the index is at 78 vs 92 probably matters more than both being labeled "Extreme Greed."
...
chart1_efficiency_by_sentiment.png
chart2_volume_vs_pnl_segment.png
chart3_segment_winrate_efficiency.png
chart4_heatmap_account_sentiment.png
